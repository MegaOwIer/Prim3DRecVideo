...
color_list = np.array([[.7, .7, .6], [.7, .5, .5], [.5, .5, .7], [.5, .55, .3], [.3, .5, .55], \
                       [1, 0.855, 0.725], [0.588, 0.804, 0.804], [1, 0.757, 0.757], [0.933, 0.474, 0.258],
                       [0.847, 191 / 255, 0.847], [0.941, 1, 1]])
...
def rotate_view_weak_perspective(verts, rx=30, ry=0, img_shape=[512, 512], expand_ratio=1.2, bbox3D_center=None,
                                 scale=None):
...
#     Rx_mat = get_rotate_x_mat(rx).type(dtype).to(device)
#     Ry_mat = get_rotate_y_mat(ry).type(dtype).to(device)
#     # print ('verts.size is', verts.size(), ', Rx_mat.size is', Ry_mat.size())
#     # os._exit(0)
#     verts_rot = torch.einsum('bij,kj->bik', verts, Rx_mat)
#     verts_rot = torch.einsum('bij,kj->bik', verts_rot, Ry_mat)
#
...
def rotate_view_perspective(verts, rx=30, ry=0, FOV=60, bbox3D_center=None, depth=None):
    ...
    h_verts, o_verts = verts
    h_verts = torch.Tensor(h_verts).cuda()
    o_verts = torch.Tensor(o_verts).cuda()
    # h_verts = h_verts.squeeze(0)
    # o_verts = o_verts.squeeze(0)
    h_num = h_verts.size(0)
    o_num = o_verts.size(0)

    verts = torch.cat([h_verts, o_verts], dim=0).unsqueeze(0)
    ...
    human_verts_aligned = verts_aligned[:, :h_num]
    object_verts_aligned = verts_aligned[:, h_num:(h_num+o_num)]

    # print (human_verts_aligned.size(), object_verts_aligned.size(), h_num, o_num)
    # os._exit(0)

    return human_verts_aligned, object_verts_aligned


def rotate_view_perspective_my(verts_list, rx=30, ry=0, FOV=60, bbox3D_center=None, depth=None):

    verts_list_cuda = []
    for vs in verts_list:
        vs = torch.Tensor(vs).cuda()
        verts_list_cuda.append(vs)

    device, dtype = verts_list_cuda[0].device, verts_list_cuda[0].dtype
    ...
    # os._exit(0)
    verts_rot_list = []
    min_value = torch.Tensor([1e9, 1e9, 1e9]).reshape(1, 3).cuda()
    max_value = torch.Tensor([1e-9, 1e-9, 1e-9]).reshape(1, 3).cuda()
    for vs in verts_list_cuda:
        # print ('vs.size is', vs.size())
        # os._exit(0)
        verts_rot = torch.einsum('bij,kj->bik', vs, Rx_mat)
        verts_rot = torch.einsum('bij,kj->bik', verts_rot, Ry_mat)
        verts_rot_list.append(verts_rot)

        flatten_verts = verts_rot.view(-1, 3)
        # print (flatten_verts.min(0).values.size(), min_value.size())
        tmp_cat = torch.cat([flatten_verts.min(0).values.unsqueeze(0), min_value], dim=0)
        # print (tmp_cat.size())
        # os._exit(0)
        min_value = tmp_cat.min(0).values.unsqueeze(0)
        tmp_cat = torch.cat([flatten_verts.max(0).values.unsqueeze(0), max_value], dim=0)
        max_value = tmp_cat.max(0).values.unsqueeze(0)

    min_value = min_value.squeeze(0)
    max_value = max_value.squeeze(0)
    # print ('max_value.size is', max_value.size())
    bbox3D_center = 0.5 * (min_value + max_value)[None, None]
    # print ('bbox.size is', bbox3D_center.size())
    # os._exit(0)

    vert_aligned_list = []
    min_dist = torch.Tensor([1e9, 1e9, 1e9]).reshape(1, 3).cuda()
    max_dist = torch.Tensor([1e-9, 1e-9, 1e-9]).reshape(1, 3).cuda()
    for vs in verts_rot_list:
        # print ('vs.size is', vs.size())
        # os._exit(0)
        vs_aligned = torch.sub(vs, bbox3D_center[:, :, :].repeat(1, vs.size(1), 1))
        vert_aligned_list.append(vs_aligned)
        tmp_cat = torch.cat([min_dist, torch.abs(vs_aligned.view(-1, 3).min(0).values.unsqueeze(0))], dim=0)
        min_dist = tmp_cat.min(0).values.unsqueeze(0)
        tmp_cat = torch.cat([max_dist, torch.abs(vs_aligned.view(-1, 3).max(0).values.unsqueeze(0))], dim=0)
        max_dist = tmp_cat.max(0).values.unsqueeze(0)

    min_dist = min_dist.squeeze(0)
    max_dist = max_dist.squeeze(0)
    # print ('max_dist.size is', max_dist.size(), ', min_dist.size is', min_dist.size())
    # os._exit(0)
    z = max_dist[:2].max() / np.tan(np.radians(FOV / 2)) + min_dist[2]
    depth = torch.tensor([[[0, 0, z]]], device=device)

    verts_align_adddepth_list = []
    for vs in vert_aligned_list:
        vs = vs + depth * 3
        vs = vs.cpu().detach().data.numpy()
        verts_align_adddepth_list.append(vs)

    return verts_align_adddepth_list


def get_rendering_results(result_image, org_depth_map, verts_shifted, offsets, faces, renderer, depth_map_thresh=1,
                          visible_weight=0.9):
    ...
    result_image[:, crop_start:crop_end] = pad_rendered_img * valid_mask + result_image[:,
                                                                           crop_start:crop_end] * valid_mask * (
                                                       1 - visible_weight) + (1 - valid_mask) * result_image[:,
                                                                                                crop_start:crop_end]
...
def rendering_mesh_rotating_view(vert_trans, renderer, triangles, image, background, internal=5):
    ...
    for azimuth_angle, elevation_angle in zip(azimuth_angles, elevation_angles):
        verts_rotated, bbox3D_center, move_depth = rotate_view_perspective(verts_rotated, rx=0, ry=azimuth_angle,
                                                                           depth=move_depth)
        rendered_image, rend_depth = renderer(verts_rotated.cpu().numpy(), triangles, background,
                                              mesh_colors=np.array([[0.9, 0.9, 0.8]]), camera_pose=camera_pose)
...
smpl24_connMat = np.array(
    [0, 1, 0, 2, 0, 3, 1, 4, 4, 7, 7, 10, 2, 5, 5, 8, 8, 11, 3, 6, 6, 9, 9, 12, 12, 15, 12, 13, 13, 16, 16, 18, 18, 20,
     20, 22, 12, 14, 14, 17, 17, 19, 19, 21, 21, 23]).reshape(-1, 2)
        ...
        if len(pt) > 1:
            if pt[0] > 0 and pt[1] > 0:
                ...
                if label_kp_order and i in bones:
                    img = cv2.putText(image, str(i), (int(pt[0]), int(pt[1])), cv2.FONT_HERSHEY_COMPLEX, 1,
                                      (255, 215, 0), 1)
...
def pymaf_draw_skeleton(image, pts, bones=smpl24_connMat, cm=None, label_kp_order=False, r=8):
    # print ('pts.shape is', pts.shape)
    # os._exit(0)

    image = image.squeeze(0)
    image = image.cpu().detach().data.numpy()
    # print (image.shape)
    # os._exit(0)
...
def pymaf_draw_skeleton_multiperson(image, pts_group, colors):
    for ind, pts in enumerate(pts_group):
        image = pymaf_draw_skeleton(image, pts, cm=colors[ind])
    return image
