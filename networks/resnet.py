...
from .residual_block import ResidualBlock, ResidualBlockMLP
...
class ResNetMLP(nn.Module):
    ...
    def __init__(self, in_channels, mid_channels, out_channels,
        ...
        self.double_after_norm = double_after_norm
        self.in_linear = nn.Linear(in_channels * 2, mid_channels)
        self.in_skip = nn.Linear(mid_channels, mid_channels)

        self.blocks = nn.ModuleList([ResidualBlockMLP(mid_channels, mid_channels)
                                     for _ in range(num_blocks)])
        self.skips = nn.ModuleList([nn.Linear(mid_channels, mid_channels)
                                    for _ in range(num_blocks)])

        self.out_norm = nn.BatchNorm1d(mid_channels)
        self.out_linear = nn.Linear(mid_channels, out_channels)
